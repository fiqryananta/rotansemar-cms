...
export default function JenisKebutuhansEdit({
    jenisKebutuhan,
    jenisPenanganans,
}: Props) {
    const { data, setData, patch, processing, errors } = useForm({
        name: jenisKebutuhan.name,
        jenis_penanganan_ids: jenisKebutuhan.jenisPenanganans.map(
            (item) => item.id,
        ),
    });
    ...
    const handleSubmit = (event: React.FormEvent) => {
        ...
        patch(route('jenis-kebutuhans.update', jenisKebutuhan.id));
    };

    return (
        <>
            <Head title="Edit Jenis Kebutuhan" />
                    ...
                    <div className="mb-8 flex items-start gap-4">
                        ...
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900">
                                Edit Jenis Kebutuhan
                            </h1>
                            <p className="mt-2 text-gray-600">
                                Perbarui jenis kebutuhan dan relasi jenis
                                penanganan.
                            </p>
                        </div>
                    </div>
