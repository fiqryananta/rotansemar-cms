        ...
        <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            <div class="mb-8 flex items-start gap-4">
                ...
                <div>
                    ...
                    <p class="mt-2 text-gray-600">Perbarui nama permission.</p>
                </div>
            </div>

            <form method="POST" action="{{ route('permissions.update', $permission->id) }}" class="space-y-4 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                @csrf
                @method('PATCH')

                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700">Permission Name</label>
                    <input id="name" name="name" value="{{ old('name', $permission->name) }}" placeholder="users.view" class="mt-1 h-10 w-full rounded-md border border-gray-300 px-3 font-mono text-sm" />
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <button type="submit" class="inline-flex h-10 flex-1 items-center justify-center rounded-md bg-rose-600 text-sm font-medium text-white transition hover:bg-rose-700">Update Permission</button>
                </div>
            </form>
        </div>
    </div>
