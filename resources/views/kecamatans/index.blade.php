        ...
        <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            <div class="mb-8 flex items-start gap-4">
                <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-emerald-50 text-emerald-600">
                    <i class="ti ti-map-2" style="font-size:1.5rem;" aria-hidden="true"></i>
                </div>
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Kecamatan</h1>
                </div>
            </div>
            ...
            <div class="rounded-lg border border-gray-200 bg-white shadow-sm">
                <div class="border-b border-gray-200 p-6">
                    <form id="filter-form" method="GET" action="{{ route('kecamatans.index') }}" class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div class="relative flex-1 sm:max-w-xs">
                            ...
                            <input id="search" type="text" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Cari Kecamatan" class="h-10 w-full rounded-md border border-gray-300 pl-10 pr-3 text-sm text-gray-900 placeholder-gray-400 focus:border-emerald-500 focus:outline-none" />
                        </div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <a href="{{ route('kecamatans.create') }}" class="inline-flex h-10 items-center gap-2 rounded-md bg-emerald-600 px-4 text-sm font-medium text-white transition hover:bg-emerald-700">
                    ...
                    <div class="overflow-x-auto rounded-md border border-gray-100">
                        <table class="min-w-full divide-y divide-gray-200 text-sm">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-4 py-3 text-left font-semibold text-gray-700">Nama Kecamatan</th>
                                    <th class="px-4 py-3 text-left font-semibold text-gray-700">Jumlah Kelurahan</th>
                            ...
                            <tbody class="divide-y divide-gray-100 bg-white">
                                @forelse ($kecamatans as $kecamatan)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-4 py-3 font-medium text-gray-900">{{ $kecamatan->name }}</td>
                                        <td class="px-4 py-3">
                                            <span class="inline-flex items-center rounded-md bg-gray-100 px-2 py-1 text-xs font-medium text-gray-700">{{ $kecamatan->kelurahans_count ?? 0 }}</span>
                                        </td>
                                        <td class="px-4 py-3 text-right">
                                            <div class="flex justify-end gap-2">
                                                <a href="{{ route('kecamatans.edit', $kecamatan->id) }}" class="inline-flex h-9 items-center rounded-md border border-gray-300 bg-white px-3 text-sm font-medium text-gray-700 transition hover:bg-gray-50">Edit</a>
                                                <form method="POST" action="{{ route('kecamatans.destroy', $kecamatan->id) }}" data-confirm="Yakin ingin menghapus Kecamatan {{ addslashes($kecamatan->name) }}?">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="px-4 py-8 text-center text-gray-500">Belum Ada Kecamatan</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                ...
                @if ($links && $links->count() > 3)
                    <div class="mt-4 flex flex-col gap-3 border-t border-gray-200 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
                        <p class="text-sm text-gray-600">
                            Menampilkan <span class="font-medium">{{ $kecamatans->firstItem() ?? 0 }}</span> -
                            <span class="font-medium">{{ $kecamatans->lastItem() ?? 0 }}</span> dari
                            <span class="font-medium">{{ $kecamatans->total() }}</span> data
                        </p>
