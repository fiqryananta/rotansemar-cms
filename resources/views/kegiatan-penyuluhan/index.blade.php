    ...
    <div class="rounded-2xl border border-gray-200/70 bg-gray-50 shadow-sm">
        <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            <div class="mb-8 flex items-start gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Kegiatan Penyuluhan</h1>
                </div>
            </div>
            ...
            <div class="rounded-lg border border-gray-200 bg-white shadow-sm">
                <div class="border-b border-gray-200 p-6">
                    <form id="filter-form" method="GET" action="{{ route('kegiatan-penyuluhan.index') }}" class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div class="relative flex-1 sm:max-w-xs">
                            ...
                            <input id="search" type="text" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Cari Kegiatan" class="h-10 w-full rounded-md border border-gray-300 pl-10 pr-3 text-sm text-gray-900 placeholder-gray-400 focus:border-cyan-500 focus:outline-none" />
                        </div>
                        <div class="flex items-center gap-2">
                            <label for="per_page" class="text-sm text-gray-600">Per halaman</label>
                            <select id="per_page" name="per_page" class="h-10 rounded-md border border-gray-300 px-3 text-sm text-gray-700 focus:border-cyan-500 focus:outline-none">
                                @foreach ([10, 25, 50] as $size)
                                    <option value="{{ $size }}" @selected(($filters['per_page'] ?? 10) == $size)>{{ $size }}</option>
                                @endforeach
                            </select>
                            <a href="{{ route('kegiatan-penyuluhan.create') }}" class="inline-flex h-10 items-center gap-2 rounded-md bg-cyan-600 px-4 text-sm font-medium text-white transition hover:bg-cyan-700">
                    ...
                    <div class="overflow-x-auto rounded-md border border-gray-100">
                        <table class="min-w-full divide-y divide-gray-200 text-sm">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-4 py-3 text-left font-semibold text-gray-700">Nama Kegiatan</th>
                                    <th class="px-4 py-3 text-left font-semibold text-gray-700">Tanggal</th>
                                    <th class="px-4 py-3 text-left font-semibold text-gray-700">Lokasi</th>
                                    <th class="px-4 py-3 text-left font-semibold text-gray-700">Sasaran</th>
                                    <th class="px-4 py-3 text-left font-semibold text-gray-700">Jumlah Sasaran</th>
                            ...
                            <tbody class="divide-y divide-gray-100 bg-white">
                                @forelse ($kegiatanPenyuluhans as $item)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-4 py-3 font-medium text-gray-900">{{ $item->nama_kegiatan }}</td>
                                        <td class="px-4 py-3 text-gray-700">{{ $item->tanggal_kegiatan }}</td>
                                        <td class="px-4 py-3 text-gray-700">{{ $item->lokasi_kegiatan }}</td>
                                        <td class="px-4 py-3 text-gray-700">{{ $item->sasaran }}</td>
                                        <td class="px-4 py-3 text-gray-700">{{ $item->jumlah_sasaran }}</td>
                                        <td class="px-4 py-3 text-gray-700">{{ is_array($item->foto_kegiatan ?? null) ? count($item->foto_kegiatan) : 0 }} foto</td>
                                        <td class="px-4 py-3 text-right">
                                            <div class="flex justify-end gap-2">
                                                <a href="{{ route('kegiatan-penyuluhan.edit', $item->id) }}" class="inline-flex h-9 items-center rounded-md border border-gray-300 bg-white px-3 text-sm font-medium text-gray-700 transition hover:bg-gray-50">Edit</a>
                                                <form method="POST" action="{{ route('kegiatan-penyuluhan.destroy', $item->id) }}" onsubmit="return confirm('Yakin ingin menghapus kegiatan ini?');">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="px-4 py-8 text-center text-gray-500">Belum Ada Kegiatan Penyuluhan</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                ...
                @if ($links && $links->count() > 3)
                    <div class="mt-4 flex flex-col gap-3 border-t border-gray-200 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
                        <p class="text-sm text-gray-600">
                            Menampilkan <span class="font-medium">{{ $kegiatanPenyuluhans->firstItem() ?? 0 }}</span> -
                            <span class="font-medium">{{ $kegiatanPenyuluhans->lastItem() ?? 0 }}</span> dari
                            <span class="font-medium">{{ $kegiatanPenyuluhans->total() }}</span> data
                        </p>
