@php($title = 'Tambah Jenis Kebutuhan')
    ...
    <div class="rounded-2xl border border-gray-200/70 bg-gray-50 shadow-sm">
        <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
            <div class="mb-8 flex items-start gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Tambah Jenis Kebutuhan</h1>
                    <p class="mt-2 text-gray-600">Tambahkan jenis kebutuhan dan pilih relasi jenis penanganan.</p>
                </div>
            </div>

            <form method="POST" action="{{ route('jenis-kebutuhans.store') }}" class="space-y-4 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                @csrf

                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700">Nama Kebutuhan</label>
                    <input id="name" name="name" value="{{ old('name') }}" placeholder="Masukkan nama kebutuhan" class="mt-1 h-10 w-full rounded-md border border-gray-300 px-3 text-sm" />
                        ...
                        @foreach ($jenisPenanganans as $jenis)
                            <label for="jenis-{{ $jenis->id }}" class="flex cursor-pointer items-start gap-3 rounded-md bg-white px-3 py-2">
                                <input
                                    ...
                                    value="{{ $jenis->id }}"
                                    @checked(in_array($jenis->id, old('jenis_penanganan_ids', [])))
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <button type="submit" class="inline-flex h-10 flex-1 items-center justify-center rounded-md bg-amber-600 text-sm font-medium text-white transition hover:bg-amber-700">Simpan Jenis Kebutuhan</button>
                </div>
            </form>
        </div>
    </div>
