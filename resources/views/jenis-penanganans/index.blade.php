                    ...
                    <div class="overflow-x-auto rounded-md border border-gray-100">
                        <table class="min-w-full divide-y divide-gray-200 text-sm">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-4 py-3 text-left font-semibold text-gray-700">Nama Jenis Penanganan</th>
                                    <th class="px-4 py-3 text-left font-semibold text-gray-700">OPD Berwenang</th>
                                    <th class="px-4 py-3 text-right font-semibold text-gray-700">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100 bg-white">
                                @forelse ($jenisPenanganans as $item)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-4 py-3 font-medium text-gray-900">{{ $item->name }}</td>
                                        <td class="px-4 py-3">
                                            @if ($item->opds->count() > 0)
                                                <div class="flex flex-wrap gap-2">
                                                    @foreach ($item->opds as $opd)
                                                        <span class="inline-flex items-center rounded-md bg-gray-100 px-2 py-1 text-xs font-medium text-gray-700">{{ $opd->name }}</span>
                                                    @endforeach
                                                </div>
                                            @else
                                                <span class="text-sm text-gray-500">Belum ada OPD</span>
                                            @endif
                                        </td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="px-4 py-8 text-center text-gray-500">Belum Ada Jenis Penanganan</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                ...
                @if ($links && $links->count() > 3)
                    <div class="mt-4 flex flex-col gap-3 border-t border-gray-200 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
                        <p class="text-sm text-gray-600">
                            Menampilkan <span class="font-medium">{{ $jenisPenanganans->firstItem() ?? 0 }}</span> -
                            <span class="font-medium">{{ $jenisPenanganans->lastItem() ?? 0 }}</span> dari
                            <span class="font-medium">{{ $jenisPenanganans->total() }}</span> data
                        </p>
