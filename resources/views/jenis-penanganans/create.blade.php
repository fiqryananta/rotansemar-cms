        ...
        <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
            <div class="mb-8 flex items-start gap-4">
                ...
                <div>
                    ...
                    <p class="mt-2 text-gray-600">Tambahkan jenis penanganan dan pilih OPD berwenang.</p>
                </div>
            </div>

            <form method="POST" action="{{ route('jenis-penanganans.store') }}" class="space-y-4 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                @csrf

                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700">Nama Jenis Penanganan</label>
                    <input id="name" name="name" value="{{ old('name') }}" placeholder="Masukkan nama jenis penanganan" class="mt-1 h-10 w-full rounded-md border border-gray-300 px-3 text-sm" />
                        ...
                        @foreach ($opds as $opd)
                            <label for="opd-{{ $opd->id }}" class="flex cursor-pointer items-start gap-3 rounded-md bg-white px-3 py-2">
                                <input
                                    ...
                                    name="opd_ids[]"
                                    value="{{ $opd->id }}"
                                    @checked(in_array($opd->id, old('opd_ids', [])))
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <button type="submit" class="inline-flex h-10 flex-1 items-center justify-center rounded-md bg-indigo-600 text-sm font-medium text-white transition hover:bg-indigo-700">Simpan Jenis Penanganan</button>
                </div>
            </form>
        </div>
    </div>
