...
@section('content')
    <div class="rounded-2xl border border-gray-200/70 bg-gray-50 shadow-sm">
        <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('users.update', $user->id) }}" class="space-y-4 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                @csrf
                @method('PATCH')

                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700">Full Name</label>
                    <input id="name" name="name" value="{{ old('name', $user->name) }}" class="mt-1 h-10 w-full rounded-md border border-gray-300 px-3 text-sm" />
                    @error('name')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label for="email" class="block text-sm font-medium text-gray-700">Email Address</label>
                    <input id="email" type="email" name="email" value="{{ old('email', $user->email) }}" class="mt-1 h-10 w-full rounded-md border border-gray-300 px-3 text-sm" />
                    @error('email')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label for="password" class="block text-sm font-medium text-gray-700">Password Baru (Opsional)</label>
                    <input id="password" type="password" name="password" placeholder="Kosongkan jika tidak diubah" class="mt-1 h-10 w-full rounded-md border border-gray-300 px-3 text-sm" />
                    @error('password')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label for="password_confirmation" class="block text-sm font-medium text-gray-700">Confirm Password</label>
                    <input id="password_confirmation" type="password" name="password_confirmation" class="mt-1 h-10 w-full rounded-md border border-gray-300 px-3 text-sm" />
                </div>
                ...
                <div>
                    ...
                    <select id="role_id" name="role_id" class="mt-1 h-10 w-full rounded-md border border-gray-300 bg-white px-3 text-sm">
                        ...
                        @foreach ($roles as $role)
                            <option value="{{ $role->id }}" @selected((string) old('role_id', $userRoleId) === (string) $role->id)>{{ $role->name }}</option>
                        @endforeach
                    </select>
                ...
                <div id="scope-opd" class="hidden">
                    ...
                    <select id="opd_id" name="opd_id" class="mt-1 h-10 w-full rounded-md border border-gray-300 bg-white px-3 text-sm">
                        ...
                        @foreach ($opds as $item)
                            <option value="{{ $item->id }}" @selected((string) old('opd_id', $user->opd_id) === (string) $item->id)>{{ $item->name }}</option>
                        @endforeach
                    </select>
                ...
                <div id="scope-faskes" class="hidden">
                    ...
                    <select id="faskes_id" name="faskes_id" class="mt-1 h-10 w-full rounded-md border border-gray-300 bg-white px-3 text-sm">
                        ...
                        @foreach ($faskes as $item)
                            <option value="{{ $item->id }}" @selected((string) old('faskes_id', $user->faskes_id) === (string) $item->id)>{{ $item->name }}</option>
                        @endforeach
                    </select>
                ...
                <div id="scope-puskesmas" class="hidden">
                    ...
                    <select id="puskesmas_id" name="puskesmas_id" class="mt-1 h-10 w-full rounded-md border border-gray-300 bg-white px-3 text-sm">
                        ...
                        @foreach ($puskesmas as $item)
                            <option value="{{ $item->id }}" @selected((string) old('puskesmas_id', $user->puskesmas_id) === (string) $item->id)>{{ $item->name }}</option>
                        @endforeach
                    </select>
                ...
                <div id="scope-kecamatan" class="hidden">
                    ...
                    <select id="kecamatan_id" name="kecamatan_id" class="mt-1 h-10 w-full rounded-md border border-gray-300 bg-white px-3 text-sm">
                        ...
                        @foreach ($kecamatans as $item)
                            <option value="{{ $item->id }}" @selected((string) old('kecamatan_id', $user->kecamatan_id) === (string) $item->id)>{{ $item->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <button type="submit" class="inline-flex h-10 flex-1 items-center justify-center rounded-md bg-sky-600 text-sm font-medium text-white transition hover:bg-sky-700">Update User</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        (function () {
            ...
            function hydrateKelurahanOptions() {
                ...
                const selectedValue = '{{ old('kelurahan_id', $user->kelurahan_id) }}';
