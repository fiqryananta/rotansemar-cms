                    ...
                    <form id="filter-form" method="GET" action="{{ route('users.index') }}" class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div class="relative flex-1 sm:max-w-xs">
                            ...
                            <input
                                ...
                                class="h-10 w-full rounded-md border border-gray-300 pl-10 pr-3 text-sm text-gray-900 placeholder-gray-400 focus:border-sky-500 focus:outline-none"
                            />
                        </div>

                        <div class="flex items-center gap-2">
                            <label for="per_page" class="text-sm text-gray-600">Per halaman</label>
                            <select id="per_page" name="per_page" class="h-10 rounded-md border border-gray-300 px-3 text-sm text-gray-700 focus:border-sky-500 focus:outline-none">
                                ...
                                @forelse ($users as $user)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex flex-wrap gap-2">
                                                @forelse ($user->roles as $role)
                                                    <span class="inline-flex items-center rounded-md border border-gray-200 bg-white px-2 py-1 text-xs font-medium text-gray-700">{{ $role->name }}</span>
                                                @empty
                                        ...
                                        <td class="px-4 py-3 text-gray-600">
                                            {{ $user->opd->name ?? $user->faskes->name ?? $user->puskesmas->name ?? $user->kecamatan->name ?? $user->kelurahan->name ?? '-' }}
                                        </td>
                                        <td class="px-4 py-3 text-right">
                                            <div class="flex justify-end gap-2">
                                                <a href="{{ route('users.edit', $user->id) }}" class="inline-flex h-9 items-center gap-2 rounded-md border border-gray-300 bg-white px-3 text-sm font-medium text-gray-700 transition hover:bg-gray-50">
                                                    Edit
                                                </a>
                        ...
                        <nav class="flex flex-wrap gap-1">
                            @foreach ($links as $link)
                                @if ($link['url'])
                                    <a
                                        href="{{ $link['url'] }}"
                                        class="rounded-md border px-3 py-1.5 text-sm transition-colors {{ $link['active'] ? 'border-sky-500 bg-sky-50 font-medium text-sky-700' : 'border-gray-200 text-gray-600 hover:bg-gray-50' }}"
                                    >
                                        {!! $link['label'] !!}
                                    </a>
                                @else
...
@push('scripts')
    <script>
        (function () {
            ...
            if (!searchInput || !form || !perPageInput) {
                return;
            }
